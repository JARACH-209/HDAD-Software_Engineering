    ...
    if request.method == 'POST':
        ...
        if form.validate_on_submit():
            ...
            new_user = User(DocName=form.DocName.data, DocId=ID,
                            DocHospital=form.DocHospital.data, password=hashed_password)
            ...
            return render_template('DocId.html', ID=ID, name=form.DocName.data)
            # return '<h1>New user has been created! Your Username is : ' + ID + '</h1>'
        else:
            return render_template('register.html', form=form,x=1)
    else:
        if current_user.is_authenticated :
            return redirect(url_for('dashboard'))
        else:
            return render_template('register.html', form=form,x=0)
