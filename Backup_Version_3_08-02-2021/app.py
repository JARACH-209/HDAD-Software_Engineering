...
def profile():
    ...
    if request.method == 'POST':
        if request.form.get("password"):
            ...
            mail.send(msg)
            try :
                query_db("delete from Authorization where DocId ='"+current_user.DocId+"' ;")
            except:
                print("error")
            
            new_user = Authorization(
                DocId=current_user.DocId,
                Token=token)
            ...
            if passwordform.validate_on_submit():
                print(passwordform.password.data)
            ...
            if form.validate_on_submit():
                ...
                creation_time = query_db("select TokenTime from Authorization where DocId ='"+current_user.DocId+"' ;")
                ...
                stored_token=query_db("select Token from Authorization where DocId ='"+current_user.DocId+"' ;")
                stored_token = np.array(stored_token)
                stored_token=stored_token[0,0]
                stored_token = str(stored_token)
                time_taken = datetime.utcnow-creation_time
                minutes = time_taken.seconds / 60
                if minutes > 1:
                    query_db("delete from Authorization where DocId ='"+current_user.DocId+"' ;")
                    return render_template('profile.html', name=current_user.DocName,message="Security Key Expired ")
                else :
                    if check_password_hash(stored_token, security_key):
                        return render_template('profile-password-input.html', name=current_user.DocName,message=minutes,form=passwordform)
            return render_template('profile.html', name=current_user.DocName,message=minutes)
    else:   
...
def dashboard():
    ...
    if request.method == 'POST':
        ...
        return render_template('dashboard.html', name=current_user.DocName, form=form,data=data,diagdata=Diagdata,chf=chf)
        ...
        if chfform.validate_on_submit() and  request.form.get("save"):
            ...
            if x == 1:
               message="Patient Have Congestive Heart Failure"
                ...
                message="error"
            return render_template('patientdetails.html',name=current_user.DocName,data=Patientdata,diagdata=diag,dd=lastDiagDate,chfform=chfform, hfform=hfform,form=2, message=message,message2="",alert=alert,chf=chf,featureform=featureform)
            ...
            if x == 1:
               message2="Patient Have Heart Failure"
            elif x == 0:
                message2="Patient Does Not Have  Heart Failure"
            else:
                message2="error"
            return render_template('patientdetails.html',name=current_user.DocName,data=Patientdata,diagdata=diag,dd=lastDiagDate,chfform=chfform, hfform=hfform,form=3, message="",message2=message2,alert=alert,chf=chf,featureform=featureform)
            ...
            if featureform.Feature2.data=='Date':
                ...
                image = create_graphs(pid,fname,fdata,date)
                print("\n")
            elif featureform.Feature2.data in common_feature and featureform.Feature1.data in common_feature :
                ...
                feature2_data = feature1_data_hf+feature2_data_chf
                ...
                print(pid)
                print("------------")
                image = create_graphs(pid,f1name,fdata,date,f2name)
                print("\n")
            elif featureform.Feature1.data in chf_feature and featureform.Feature2.data in chf_feature:
                feature1_data = query_db('select '+ featureform.Feature1.data+" from CHF where PatientId='"+pid+"';")
                feature2_data = query_db('select '+ featureform.Feature2.data+" from CHF where PatientId='"+pid+"';")
                
                # feature_data=feature_data.flatten()
                fdata = np.array(feature1_data)
                fdata = fdata[:,0]
                date = np.array(feature2_data)
                date = date[:,0]
                f1name=featureform.Feature1.data
                f2name =featureform.Feature2.data
                print(fdata)
                ...
                print("------------")
                image = create_graphs(pid,f1name,fdata,date,f2name)
            elif featureform.Feature1.data in hf_feature and featureform.Feature2.data in hf_feature:
                feature1_data = query_db('select '+ featureform.Feature1.data+" from HF where PatientId='"+pid+"';")
                feature2_data = query_db('select '+ featureform.Feature2.data+" from HF where PatientId='"+pid+"';")
                ...
                image = "Statstics cannot be shown"
            return render_template('patientdetails.html',name=current_user.DocName,data=Patientdata,diagdata=diag,dd=lastDiagDate,chfform=chfform, hfform=hfform, form=4, message="",message2="",alert=alert,chf=chf,image=image,featureform=featureform)
    return render_template('patientdetails.html',name=current_user.DocName,data=Patientdata,diagdata=diag,dd=lastDiagDate,chfform=chfform, hfform=hfform, form=0, message="",message2="",alert=alert,chf=chf,featureform=featureform)
    ...
    if request.method == 'POST':
        ...
        if form.validate_on_submit():
            ...
            if Email:
                ...
                new_user = User(DocName=form.DocName.data, DocId=ID,
                                DocHospital=form.DocHospital.data,Email= form.Email.data,password=hashed_password)
                db.session.add(new_user)
                db.session.commit()
                f = open(path, 'w')
